...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<select name="f[user_id][in][]" class="select-model form-control" data-model="admin/member" data-text="{{username}}" data-params='{"f[role_id][in][]":"1"}' data-term="title" data-placeholder="请输入用户名" value="<{$_filters.user_id.in|default:[]|implode:','}>"></select>
		</div>
	</div>
	<div class="form-group col-sm-4">
